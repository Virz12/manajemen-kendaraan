...
<head>
    ...
    <title>{{ config('app.name') }} | Admin | Ubah Data Pegawai</title>
</head>
<body>  
                ...
                <div class="row g-4">
                    <div class="col-12">
                        <div class="bg-light text-center rounded p-4">
                            ...
                            {{-- Form --}}
                            <form action="{{ route('admin.data.pegawai.update', ['pegawai' => $pegawai]) }}" method="POST" enctype="multipart/form-data" class="row g-3">
                                @csrf
                                @method('PUT')
                                <div class="col-md-6">
                                    <label for="nip" class="form-label w-100 text-start">NIP<span class="text-danger">*</span></label>
                                    <input type="number" value="{{ $pegawai->nip }}" name="nip" class="form-control @error('nip') is-invalid @enderror" id="nip" min="1" @required(true)>
                                    @error('nip')
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="text" value="{{ $pegawai->nama }}" name="nama" class="form-control @error('nama') is-invalid @enderror" id="nama" autocomplete="off" @required(true)>
                                    @error('nama')
                                        <div class="text-danger text-start"><small>{{ $message }}</small></div>
                                    @enderror
                                </div>
                                <div class="col-12">
                                    ...
                                    <input type="text" value="{{ $pegawai->username }}" name="username" class="form-control @error('username') is-invalid @enderror" id="username" autocomplete="off" @required(true)>
                                    @error('username')
                                        <div class="text-danger text-start"><small>{{ $message }}</small></div>
                                    @enderror
                                </div>
                                ...
                                <div class="col-md-4">
                                    ...
                                    <select id="Role" name="kelompok" class="form-select @error('kelompok') is-invalid @enderror" @required(true)>
                                        <option value="{{ $pegawai->kelompok }}" selected hidden>{{ $pegawai->kelompok }}</option>
                                ...
                                <div class="col-md-4">
                                    ...
                                    <select id="Status" name="status" class="form-select @error('status') is-invalid @enderror" @required(true)>
                                        <option value="{{ $pegawai->status }}" selected hidden>{{ $pegawai->status }}</option>
