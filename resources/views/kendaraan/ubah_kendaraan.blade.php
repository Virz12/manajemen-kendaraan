...
<head>
    ...
    <title>{{ config('app.name') }} | Kendaraaan | Ubah Data Kendaraan</title>
</head>
<body>
                ...
                <div class="row g-4">
                    <div class="col-12">
                        <div class="bg-light text-center rounded p-4">
                            ...
                            {{-- Form --}}
                            <form action="{{ route('kendaraan.data.kendaraan.update', ['kendaraan' => $datakendaraan]) }}" method="POST" enctype="multipart/form-data" class="row g-3">
                                @csrf
                                @method('PUT')
                                <div class="col-md-6">
                                    <label for="jenis_kendaraan" class="form-label w-100 text-start">Jenis Kendaraan<span class="text-danger">*</span></label>
                                    <input type="text" value="{{ $datakendaraan->jenis_kendaraan }}" name="jenis_kendaraan" class="form-control @error('jenis_kendaraan') is-invalid @enderror" id="jenis_kendaraan" min="1" autocomplete="off" @required(true)>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="number" value="{{ $datakendaraan->tahun }}" name="tahun" class="form-control @error('tahun') is-invalid @enderror" id="tahun" @required(true)>
                                    @error('tahun')
                                        <div class="text-danger text-start"><small>{{ $message }}</small></div>
                                    @enderror
                                </div>
                                <div class="col-12">
                                    ...
                                    <input type="text" value="{{ $datakendaraan->nopol }}" name="nopol" class="form-control @error('nopol') is-invalid @enderror" id="nopol" autocomplete="off" @required(true)>
                                    @error('nopol')
                                        <div class="text-danger text-start"><small>{{ $message }}</small></div>
                                    @enderror
                                </div>
                                <div class="col-12">
                                    ...
                                    <input type="text" value="{{ $datakendaraan->warna }}" name="warna" class="form-control @error('warna') is-invalid @enderror" id="warna" autocomplete="off" @required(true)>
                                    @error('warna')
                                        <div class="text-danger text-start"><small>{{ $message }}</small></div>
                                    @enderror
                                </div>
